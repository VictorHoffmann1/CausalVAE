...
for u,l in train_dataset:
  for i in range(4):
    for j in range(-5,5):
      L, kl, rec, reconstructed_image, z = lvae.negative_elbo_bound(u.to(device),l.to(device),i,sample = sample, adj=j*0)
    save_image(reconstructed_image[0], 'figs_test_vae_pendulum/reconstructed_image_{}_{}.png'.format(i, count),  value_range = (0,1)) 
